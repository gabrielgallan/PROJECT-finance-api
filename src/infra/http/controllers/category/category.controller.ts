...
export class CategoryController {
  ...
  @ApiNotFoundResponse({
    description: "No wallet categories found",
  ...
  async create(
    ...
    await this.cacheRepository.delete(`categories:${user.sub}`);
  ...
  @ApiOkResponse({
    description: "List of wallet categories",
  ...
  async list(@CurrentUser() user: UserPayload) {
    ...
    const cachedHit = await this.cacheRepository.get(cacheKey);

    if (cachedHit) {
      return {
        categories: JSON.parse(cachedHit),
      };
    }
    ...
    const categories = result.value.categories.map(CategoryPresenter.toHTTP);
  ...
  async edit(
    @CurrentUser() user: UserPayload,
    @Body(new ZodValidationPipe(editCategoryBodySchema))
    body: EditCategoryBodyDTO,
    @Param(new ZodValidationPipe(editCategoryParamsSchema))
    params: EditCategoryParamsDTO,
  ) {
    const { name, description } = body;
    const { id } = params;
